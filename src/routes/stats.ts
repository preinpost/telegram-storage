...
export function statsRoutes(deps: AppDeps, sessionSecret: string): Hono<AppEnv> {
  ...
  app.get('/', async (c) => {
    ...
    const byFolder = await deps.db.statsByFolder();

    const folderUsage: FolderUsageJson[] = [];
    for (const row of byFolder) {
      if (row.folder_id !== null) {
        const folder = await deps.db.getFolder(row.folder_id);
        if (!folder) continue; // orphaned folder_id (should not happen)
        if (rankOf(await resolveFolderRole(deps.db, user.id, row.folder_id)) < rankOf('read')) {
          continue;
        }
        folderUsage.push({
    ...
    return c.json({
      totalSize,
      fileCount: await deps.db.countActiveFiles(),
      folderCount: await deps.db.countFolders(),
      userCount: await deps.db.countUsers(),
      folderUsage,
    });
  });
